...
public class CustomButtonEvent : MonoBehaviour {
	...
	void OnPointDown(){
		...
		Shooter3DNetworkManager.instance.myPlayer.GetComponent<VehicleManager>().EnableKey (gameObject.name);
		if( onPress != null  ){
	...
	void OnPointUp(){
	    ...
	    Shooter3DNetworkManager.instance.myPlayer.GetComponent<VehicleManager>().DisableKey (gameObject.name);
		if( onPress != null  ){
			//Debug.Log("OnPointUp");
